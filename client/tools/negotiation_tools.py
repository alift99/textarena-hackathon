...
def resource_evaluator(trade: Trade, accepted: bool) -> str:
    ...
    for res, qty in offered.items():
        if accepted:
            opponent_values[res] = min(
                opponent_values.get(res, 40), 1.2 * qty
            )  # Opponent values this less
        else:
            opponent_values[res] = max(
                opponent_values.get(res, 10), 0.8 * qty
            )  # Opponent values this more
